...
try:
    ...
    logger.error(f"❌ Failed to import spaCy: {e}. Resume extraction will be limited.")
    ...
    if _nlp_instance is None:
        if SPACY_AVAILABLE:
            try:
                ...
                logger.error(f"❌ Failed to create spaCy pipeline: {e}")
                _nlp_instance = _create_mock_nlp()
        else:
            _nlp_instance = _create_mock_nlp()
    return _nlp_instance

def _create_mock_nlp():
    """Create a dummy NLP object that returns a doc with no entities."""
    class MockDoc:
    ...
    class MockNLP:
        ...
        def add_pipe(self, *args, **kwargs):
            return self
        def add_patterns(self, *args, **kwargs):
            pass
    return MockNLP()
